...
func _on_server_card_added(card_node: BaseCard, owner_id: int):
	# Convert Player Number (1 or 2) to Network Peer ID
	var target_peer_id = ConnectionManager.get_peer_id_for_player(owner_id)
	
	if target_peer_id == -1:
		printerr("NetworkInterface: Could not find peer for Player ", owner_id, " to send card draw.")
		return
		
	print("Server: Sending card draw for ", card_node.name, " to Peer ", target_peer_id)
	
	# Send the RPC only to the target player
	client_receive_card_draw.rpc_id(target_peer_id, card_node.db_key, owner_id)
